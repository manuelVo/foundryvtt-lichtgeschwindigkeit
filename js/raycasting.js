...
function wasmComputePolygon(origin, radius, { type = "sight", angle = 360, density = 6, rotation = 0, unrestricted = false } = {}) {
	...
	function logParams(force, error_fn) {
		rustifyParams(walls, type, origin, height, radius, distance, density, angle, rotation, force, error_fn);
	}
	...
	let sight;
	try {
		sight = Lichtgeschwindigkeit.computePolygon(walls, type, origin, height, radius, distance, density, angle, rotation, internals);
	}
	catch (e) {
		console.error(e);
...
function rustifyParams(walls, type, origin, height, radius, distance, density, angle, rotation, force = false, error_fn = console.warn) {
	/*if (!force) {
	...
	error_fn(Lichtgeschwindigkeit.serializeData(walls, type, origin, height, radius, distance, density, angle, rotation));
}
